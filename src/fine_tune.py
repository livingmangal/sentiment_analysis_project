...
from src.database import Feedback, ModelVersion, get_db_session
...
def fine_tune_model(batch_size: int = 8, lr: float = 0.0001, epochs: int = 3) -> Optional[str]:
    """
    ...
    unused_feedback = db_session.query(Feedback).filter(Feedback.is_used_for_training.is_(False)).all()
    ...
    labels = [float(f.actual_sentiment) for f in unused_feedback]
    ...
    # 5. Fine-tune
    # If the model has 3 outputs, it should use CrossEntropyLoss
